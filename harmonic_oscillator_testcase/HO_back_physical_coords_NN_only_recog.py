...
# We consider the extended state-space model in which the unknown frequency
# is a constant state: only a recognition model is learned, which predicts
# (x1(0), x2(0), x3(0) = frequency) for each input trajectory
    ...
    if train:
        ...
        if 'Continuous/Harmonic_Oscillator' in config.system:
            ...
            config.update(dict(
                ...
                constrain_u=[-1., 1.],
                constrain_x=[],
                grid_inf=[-1, -1, config.pulse ** 2],
                grid_sup=[1, 1, config.pulse ** 2]))
