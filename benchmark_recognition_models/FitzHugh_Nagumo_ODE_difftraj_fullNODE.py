...
# constant (known) stimulus, jointly with the unknown dynamics (full NN
# dynamics model)
...
if __name__ == '__main__':
    ...
    config = Config(system='Continuous/Benchmark_recognition/'
                           'FitzHugh_Nagumo_ODE_fullNODE/MLP2_noisy_inputs',
    ...
    if train:
        ...
        if 'KKL_u0T' in config.init_state_obs_method:
            ...
            if 'back' in config.init_state_obs_method:
                D = torch.tensor([[1.3534, -7.1499, -6.0118],
                                  [6.3538, -8.6500, -1.4215],
                                  [1.2600, -3.7842, -7.6313]])
